...
const isLinkedMode = () => {
  ...
  if (path.basename(pwd) === 'plutono-toolkit') {
    return true;
  }

  try {
    const resolvedPath = path.resolve(`${__dirname}/../../../node_modules/@credativ/plutono-toolkit`);
...
const entrypoint = () => {
  ...
  // IF we have a toolkit directory AND linked plutono toolkit AND the toolkit dir is a symbolic lik
