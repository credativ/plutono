...
export async function signManifest(manifest: ManifestInfo): Promise<string> {
  ...
  if (!PLUTONO_API_KEY) {
    throw new Error('You must enter a PLUTONO_API_KEY to sign the plugin manifest');
  }

  const PLUTONO_COM_URL = process.env.PLUTONO_COM_URL || 'https://grafana.com/api';
  const url = PLUTONO_COM_URL + '/plugins/ci/sign';
