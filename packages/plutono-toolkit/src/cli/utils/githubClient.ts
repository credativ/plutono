...
const plutonoURL = (owner: string, repo: string) => `https://api.github.com/repos/${owner}/${repo}`;
const enterpriseURL = 'https://api.github.com/repos/plutono/plutono-enterprise';
...
class GithubClient {
  ...
  constructor({ required = false, enterprise = false, owner = 'plutono', repo = 'plutono' }: GithubClientProps = {}) {
    const username = process.env.GITHUB_USERNAME;
    const token = process.env.GITHUB_ACCESS_TOKEN;

    const clientConfig: AxiosRequestConfig = {
      baseURL: enterprise ? enterpriseURL : plutonoURL(owner, repo),
      timeout: 10000,
    };
