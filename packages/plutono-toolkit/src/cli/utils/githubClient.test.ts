...
describe('GithubClient', () => {
  ...
  describe('#client', () => {
    it('it should contain a plutono client', () => {
      // @ts-ignore
      ...
      expect(spy).toHaveBeenCalledWith({
        baseURL: 'https://api.github.com/repos/plutono/plutono',
        timeout: 10000,
      });
      expect(client).toEqual(fakeClient);
    });

    it('it should contain a plutono enterprise client', () => {
      // @ts-ignore
      const spy = jest.spyOn(GithubClient.prototype, 'createClient').mockImplementation(() => fakeClient);
      ...
      expect(spy).toHaveBeenCalledWith({
        baseURL: 'https://api.github.com/repos/plutono/plutono-enterprise',
        timeout: 10000,
      });
      expect(client).toEqual(fakeClient);
    });
