...
const changelogTaskRunner = ({ milestone }: ChangelogOptions) =>
  useSpinner('Generating changelog', async () => {
    ...
    const toolkitIssues = issues.filter((item: any) =>
      item.labels.find((label: any) => label.name === 'area/plutono/toolkit')
    );
    const plutonoUiIssues = issues.filter((item: any) =>
      item.labels.find((label: any) => label.name === 'area/plutono/ui')
    );

    let markdown = '';

    markdown += getPackageChangelog('Plutono', issues);
    markdown += getPackageChangelog('plutono-toolkit', toolkitIssues);
    markdown += getPackageChangelog('plutono-ui', plutonoUiIssues);

    console.log(markdown);
  });

function getMarkdownLineForIssue(item: any) {
  const githubPlutonoUrl = 'https://github.com/grafana/grafana';
  let markdown = '';
  ...
  if (!item.pull_request) {
    markdown += '* ' + title + '.';
    markdown += ` [#${item.number}](${githubPlutonoUrl}/issues/${item.number})`;
  } else {
    markdown += '* ' + title + '.';
    markdown += ` [#${item.number}](${githubPlutonoUrl}/pull/${item.number})`;
