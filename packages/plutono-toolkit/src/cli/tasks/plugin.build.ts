...
export const prepare = () =>
  useSpinner('Preparing', () =>
    Promise.all([
      // Remove local dependencies for @credativ/plutono-data/node_modules
      // See: https://github.com/grafana/grafana/issues/26748
      rimraf(resolvePath(__dirname, 'node_modules/@credativ/plutono-data/node_modules')),
...
export const versions = async () => {
  ...
  const toolkitVersion = await execa('plutono-toolkit', ['--version']);
  console.log(`Using @credativ/plutono-toolkit ${toolkitVersion}`);
};
...
export const lintPlugin = ({ fix }: Fixable = {}) =>
  useSpinner('Linting', async () => {
    try {
      ...
      console.log('\n');
      console.log('--------------------------------------------------------------');
      console.log('NOTE: @credativ/plutono-toolkit has migrated to use eslint');
