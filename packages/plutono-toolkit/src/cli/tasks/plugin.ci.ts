 ...
 *  3. prepare plutono environment in: `~/ci/plutono-test-env`
 */
const packagePluginRunner: TaskRunner<PluginCIOptions> = async ({ signatureType, rootUrls }) => {
  ...
  const plutonoEnvDir = path.resolve(ciDir, 'plutono-test-env');
  await execa('rimraf', [packagesDir, distDir, plutonoEnvDir]);
  ...
  console.log('Setup Plutono Environment');
  let p = path.resolve(plutonoEnvDir, 'plugins', pluginInfo.id);
  ...
  p = path.resolve(plutonoEnvDir, 'custom.ini');
  const customIniBody =
...
const pluginReportRunner: TaskRunner<PluginCIOptions> = async ({ upload }) => {
  ...
  const report: PluginBuildReport = {
    ...
    plutonoVersion: getPlutonoVersions(),
  ...
  const PLUTONO_API_KEY = process.env.PLUTONO_API_KEY;
  if (!PLUTONO_API_KEY) {
    console.log('Enter a PLUTONO_API_KEY to upload the plugin report');
    return;
  }
  ...
  const info = await axios.post(url, report, {
    headers: { Authorization: 'Bearer ' + PLUTONO_API_KEY },
  });
  if (info.status === 200) {
