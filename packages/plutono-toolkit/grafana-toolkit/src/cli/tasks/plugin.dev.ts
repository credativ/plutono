...
const yarnlink = () =>
  useSpinner('Linking local toolkit', async () => {
    try {
      ...
      await execa('yarn', ['remove', '@credativ/plutono-toolkit']);
    } catch (e) {
      console.log('\n', e.message, '\n');
    }
    await execa('yarn', ['link', '@credativ/plutono-toolkit']);
    ...
    console.log('Added dependencies required by local @credativ/plutono-toolkit.  Do not checkin this package.json!');
  });
