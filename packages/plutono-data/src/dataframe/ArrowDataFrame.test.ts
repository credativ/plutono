...
import { plutonoDataFrameToArrowTable, arrowTableToDataFrame } from './ArrowDataFrame';
...
describe('Read/Write arrow Table to DataFrame', () => {
  test('should parse output with dataframe', () => {
    ...
    const table = plutonoDataFrameToArrowTable(frame);
  ...
  test('should support duplicate field names', () => {
    ...
    const table = plutonoDataFrameToArrowTable(frame, true);
    expect(table.length).toEqual(frame.length);
  ...
  test('Export arrow table names', () => {
    ...
    const t1 = plutonoDataFrameToArrowTable(simple);
    const t2 = plutonoDataFrameToArrowTable(simple, true);
