...
import { FieldConfig, FieldType, PlutonoTheme, MutableDataFrame, VizOrientation } from '@credativ/plutono-data';
...
describe('GraphNG utils', () => {
  describe('preparePlotConfigBuilder', () => {
    ...
    it.each([VizOrientation.Auto, VizOrientation.Horizontal, VizOrientation.Vertical])('orientation', (v) => {
      expect(
        preparePlotConfigBuilder(frame!, { colors: { panelBg: '#000000' } } as PlutonoTheme, {
          ...config,
          orientation: v,
        })
      ).toMatchSnapshot();
