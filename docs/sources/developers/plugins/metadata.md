...
The plugin.json file is required for all plugins. When Plutono starts, it scans the plugin folders and mounts every folder that contains a plugin.json file unless the folder contains a subfolder named dist. In that case, Plutono mounts the dist folder instead.
...
| `info`          | [object](#info)         | **Yes**  | Metadata for the plugin. Some fields are used on the plugins page in Plutono and others on grafana.com if the plugin is published.                                                                                                                                                                                                                                                                      |
...
| `executable`    | string                  | No       | The first part of the file name of the backend component executable. There can be multiple executables built for different operating system and architecture. Plutono will check for executables named `<executable>_<$GOOS>_<lower case $GOARCH><.exe for Windows>`, e.g. `plugin_linux_amd64`. Combination of $GOOS and $GOARCH can be found here: https://golang.org/doc/install/source#environment. |
...
| Property            | Type                 | Required | Description                                                                                                                   |
|---------------------|----------------------|----------|-------------------------------------------------------------------------------------------------------------------------------|
| `plutonoDependency` | string               | **Yes**  | Required Plutono version for this plugin. Validated using https://github.com/npm/node-semver.                                 |
| `plutonoVersion`    | string               | No       | (Deprecated) Required Plutono version for this plugin, e.g. `6.x.x 7.x.x` to denote plugin requires Plutono v6.x.x or v7.x.x. |
| `plugins`           | [object](#plugins)[] | No       | An array of required plugins on which this plugin depends.                                                                    |

### plugins

Plugin dependency. Used to display information about plugin dependencies in the Plutono UI.
...
Metadata for the plugin. Some fields are used on the plugins page in Plutono and others on grafana.com if the plugin is published.
...
| `description` | string                   | No       | Description of plugin. Used on the plugins page in Plutono and for search on grafana.com.                                     |
