...
aliases = ["/docs/plutono/latest/dashboards/share-dashboard/","/docs/plutono/latest/reference/share_dashboard/"]
weight = 6
+++
...
1. Go to the home page of your Plutono instance.
...
A dashboard snapshot shares an interactive dashboard publicly. Plutono strips sensitive data like queries
...
provided by Grafana Labs that allows you to publish dashboard snapshots to an external Plutono instance. The same rules still apply: anyone with the link can view it. You can set an expiration time if you want the snapshot removed after a certain time period.
...
In case you created a snapshot by mistake, click **delete snapshot** to remove the snapshot from your Plutono instance.

## Dashboard export

Plutono dashboards can easily be exported and imported. For more information, refer to [Export and import dashboards]({{< relref "../dashboards/export-import.md" >}}).
