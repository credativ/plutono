...
func GetPlutonoPluginDir(currentOS string) string {
...
// getPlutonoRoot tries to get root of directory when developing plutono, ie. repo root. It is not perfect, it just
// checks what is the binary path and tries to guess based on that, but if it is not running in dev env you get a bogus
// path back.
func getPlutonoRoot() (string, error) {
...
func returnOsDefault(currentOs string) string {
	...
	case "windows":
		return "../data/plugins"
	case "darwin":
		return "/usr/local/var/lib/plutono/plugins"
	case "freebsd":
		return "/var/db/plutono/plugins"
	case "openbsd":
		return "/var/plutono/plugins"
	default: // "linux"
		return "/var/lib/plutono/plugins"
	}
}
