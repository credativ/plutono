...
type PlutonoComClient struct {
	retryCount int
}

func (client *PlutonoComClient) GetPlugin(pluginId, repoUrl string) (models.Plugin, error) {
...
func (client *PlutonoComClient) DownloadFile(pluginName string, tmpFile *os.File, url string, checksum string) (err error) {
...
func (client *PlutonoComClient) ListAllPlugins(repoUrl string) (models.PluginRepo, error) {
...
func createRequest(repoUrl string, subPaths ...string) (*http.Request, error) {
	...
	req.Header.Set("plutono-version", plutonoVersion)
	req.Header.Set("plutono-os", runtime.GOOS)
	req.Header.Set("plutono-arch", runtime.GOARCH)
	req.Header.Set("User-Agent", "plutono "+plutonoVersion)
