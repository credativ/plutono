...
type ValiExecutor struct{}

func NewValiExecutor(dsInfo *models.DataSource) (tsdb.TsdbQueryEndpoint, error) {
	return &ValiExecutor{}, nil
}
...
func init() {
	plog = log.New("tsdb.vali")
	tsdb.RegisterTsdbQueryEndpoint("vali", NewValiExecutor)
	legendFormat = regexp.MustCompile(`\{\{\s*(.+?)\s*\}\}`)
	intervalCalculator = tsdb.NewIntervalCalculator(&tsdb.IntervalOptions{MinInterval: time.Second * 1})
}

func (e *ValiExecutor) Query(ctx context.Context, dsInfo *models.DataSource, tsdbQuery *tsdb.TsdbQuery) (*tsdb.Response, error) {
	...
	for _, query := range queries {
		...
		span, _ := opentracing.StartSpanFromContext(ctx, "alerting.vali")
...
func parseQuery(dsInfo *models.DataSource, queries []*tsdb.Query, queryContext *tsdb.TsdbQuery) ([]*ValiQuery, error) {
	qs := []*ValiQuery{}
...
func parseResponse(value *loghttp.QueryResponse, query *ValiQuery) (*tsdb.QueryResult, error) {
