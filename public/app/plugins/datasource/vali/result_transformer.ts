...
import { getTemplateSrv, getDataSourceSrv } from '@credativ/plutono-runtime';
...
/**
 * Transforms ValiStreamResult structure into a dataFrame. Used when doing standard queries and newer version of Vali.
 */
export function valiStreamResultToDataFrame(stream: ValiStreamResult, reverse?: boolean, refId?: string): DataFrame {
...
/**
 * Transform ValiResponse data and appends it to MutableDataFrame. Used for streaming where the dataFrame can be
 * a CircularDataFrame creating a fixed size rolling buffer.
 * TODO: Probably could be unified with the logStreamToDataFrame function.
 * @param response
 * @param data Needs to have ts, line, labels, id as fields
 */
export function appendResponseToBufferedData(response: ValiTailResponse, data: MutableDataFrame) {
  // Should we do anything with: response.dropped_entries?

  const streams: ValiStreamResult[] = response.streams;
...
function valiMatrixToTimeSeries(matrixResult: ValiMatrixResult, options: TransformerOptions): TimeSeries {
  const name = createMetricLabel(matrixResult.metric, options);
  return {
    target: name,
    title: name,
    datapoints: valiPointsToTimeseriesPoints(matrixResult.values, options),
...
export function valiPointsToTimeseriesPoints(
...
export function valiResultsToTableModel(
  valiResults: Array<ValiMatrixResult | ValiVectorResult>,
  resultCount: number,
  refId: string,
  meta: QueryResultMeta,
  valueWithRefId?: boolean
): TableModel {
  if (!valiResults || valiResults.length === 0) {
    return new TableModel();
  }

  // Collect all labels across all metrics
  const metricLabels: Set<string> = new Set<string>(
    valiResults.reduce((acc, cur) => acc.concat(Object.keys(cur.metric)), [] as string[])
  );
  ...
  valiResults.forEach((series) => {
    const newSeries: ValiMatrixResult = {
      metric: series.metric,
      values: (series as ValiVectorResult).value
        ? [(series as ValiVectorResult).value]
        : (series as ValiMatrixResult).values,
    };
...
// Turn vali stats { metric: value } into meta stat { title: metric, value: value }
function valiStatsToMetaStat(stats: ValiStats | undefined): QueryResultMetaStat[] {
...
export function valiStreamsToDataFrames(
  response: ValiStreamResponse,
  ...
  const stats: QueryResultMetaStat[] = valiStatsToMetaStat(response.data.stats);
  // Use custom mechanism to identify which stat we want to promote to label
  const custom = {
    valiQueryStatKey: 'Summary: total bytes processed',
  };
  ...
  const series: DataFrame[] = data.map((stream) => {
    const dataFrame = valiStreamResultToDataFrame(stream, reverse, target.refId);
 ...
 */
export const enhanceDataFrame = (dataFrame: DataFrame, config: ValiOptions | null): void => {
  if (!config) {
    return;
  }
...
export function rangeQueryResponseToTimeSeries(
  response: ValiResponse,
  query: ValiRangeQueryRequest,
  target: ValiQuery,
  responseListLength: number,
  scopedVars: ScopedVars
): TimeSeries[] {
  /** Show results of Vali metric queries only in graph */
  const meta: QueryResultMeta = {
  ...
  switch (response.data.resultType) {
    case ValiResultType.Vector:
      return response.data.result.map((vecResult) =>
        valiMatrixToTimeSeries({ metric: vecResult.metric, values: [vecResult.value] }, transformerOptions)
      );
    case ValiResultType.Matrix:
      return response.data.result.map((matrixResult) => valiMatrixToTimeSeries(matrixResult, transformerOptions));
    default:
      return [];
  }
}
...
export function processRangeQueryResponse(
  ...
  switch (response.data.resultType) {
    case ValiResultType.Stream:
      return of({
        data: valiStreamsToDataFrames(response as ValiStreamResponse, target, limit, config, reverse),
