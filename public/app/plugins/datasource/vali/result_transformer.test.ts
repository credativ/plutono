...
const streamResult: ValiStreamResult[] = [
  {
    stream: {
      foo: 'bar',
    },
...
const valiResponse: ValiStreamResponse = {
  status: 'success',
  data: {
    result: streamResult,
...
describe('vali result transformer', () => {
  beforeAll(() => {
  ...
  describe('valiStreamResultToDataFrame', () => {
    it('converts streams to series', () => {
      const data = streamResult.map((stream) => ResultTransformer.valiStreamResultToDataFrame(stream));
    ...
    it('should always generate unique ids for logs', () => {
      const streamResultWithDuplicateLogs: ValiStreamResult[] = [
        {
          stream: {
            foo: 'bar',
          },
      ...
      const data = streamResultWithDuplicateLogs.map((stream) => ResultTransformer.valiStreamResultToDataFrame(stream));
    ...
    it('should append refId to the unique ids if refId is provided', () => {
      const data = streamResult.map((stream) => ResultTransformer.valiStreamResultToDataFrame(stream, false, 'B'));
      expect(data.length).toBe(2);
  ...
  describe('valiStreamsToDataFrames', () => {
    it('should enhance data frames', () => {
      jest.spyOn(ResultTransformer, 'enhanceDataFrame');
      const dataFrames = ResultTransformer.valiStreamsToDataFrames(valiResponse, { refId: 'B' }, 500, {
        derivedFields: [
          {
  ...
  describe('appendResponseToBufferedData', () => {
    it('should return a dataframe with ts in iso format', () => {
      const tailResponse: ValiTailResponse = {
        streams: [
          {
            stream: {
  ...
  describe('valiResultsToTableModel', () => {
    it('should correctly set the type of the label column to be a string', () => {
      const valiResultWithIntLabel = ([
        { metric: { test: 1 }, value: [1610367143, 10] },
        { metric: { test: 2 }, value: [1610367144, 20] },
      ] as unknown) as ValiMatrixResult[];

      const table = ResultTransformer.valiResultsToTableModel(valiResultWithIntLabel, 1, 'A', {});
...
describe('enhanceDataFrame', () => {
  it('adds links to fields', () => {
    ...
    expect(fc.getFieldByName('trace2')!.config.links![0]).toEqual({
      title: '',
      internal: { datasourceName: 'Vali1', datasourceUid: 'uid', query: { query: 'test' } },
      url: '',
    });
    expect(fc.getFieldByName('trace2')!.config.links![1]).toEqual({
      title: '',
      internal: { datasourceName: 'Vali1', datasourceUid: 'uid2', query: { query: 'test' } },
      url: '',
    });
  });

  describe('valiPointsToTimeseriesPoints()', () => {
    /**
     ...
     * - Start/end parameters are in ns (as expected for Vali)
    ...
    it('returns data as is if step, start, and end align', () => {
      ...
      const result = ResultTransformer.valiPointsToTimeseriesPoints(data, options as TransformerOptions);
