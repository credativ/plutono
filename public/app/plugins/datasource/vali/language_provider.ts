...
export type ValiHistoryItem = HistoryItem<ValiQuery>;
...
export function addHistoryMetadata(item: CompletionItem, history: ValiHistoryItem[]): CompletionItem {
...
export default class ValiLanguageProvider extends LanguageProvider {
  labelKeys: string[];
  ...
  constructor(datasource: ValiDatasource, initialValues?: any) {
    super();
  ...
  async importQueries(queries: ValiQuery[], datasourceType: string): Promise<ValiQuery[]> {
  ...
  async fetchLabelValues(key: string): Promise<string[]> {
    const url = `/vali/api/v1/label/${key}/values`;
