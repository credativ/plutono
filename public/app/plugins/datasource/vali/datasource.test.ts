...
import { ValiQuery, ValiResponse, ValiResultType } from './types';
import { getQueryOptions } from 'test/helpers/getQueryOptions';
import { TemplateSrv } from 'app/features/templating/template_srv';
import { backendSrv } from 'app/core/services/backend_srv';
import { CustomVariableModel } from '../../../features/variables/types';
import { initialCustomVariableModelState } from '../../../features/variables/custom/reducer';
import { makeMockValiDatasource } from './mocks';
...
const testLogsResponse: FetchResponse<ValiResponse> = {
  data: {
    data: {
...
const testMetricsResponse: FetchResponse<ValiResponse> = {
  data: {
    data: {
...
describe('ValiDatasource', () => {
  ...
  describe('when creating range query', () => {
    let ds: ValiDatasource;
    let adjustIntervalSpy: jest.SpyInstance;

    beforeEach(() => {
      ds = createValiDSForTests();
  ...
  describe('when doing logs queries with limits', () => {
    const runLimitTest = async ({
      ...
      const ds = new ValiDatasource(settings, templateSrvMock, timeSrvStub as any);

      const options = getQueryOptions<ValiQuery>({ targets: [{ expr, refId: 'B', maxLines: queryMaxLines }] });
  ...
  describe('when querying', () => {
    function setup(expr: string, app: CoreApp, instant?: boolean, range?: boolean) {
      const ds = createValiDSForTests();
      const options = getQueryOptions<ValiQuery>({
    ...
    it('should return custom error message when Vali returns escaping error', async () => {
      const ds = createValiDSForTests();
      const options = getQueryOptions<ValiQuery>({
  ...
  describe('when performing testDataSource', () => {
    describe('and call succeeds', () => {
      it('should return successfully', async () => {
        ...
        const ds = createValiDSForTests({} as TemplateSrv);
    ...
    describe('and call fails with 401 error', () => {
      it('should return error status and a detailed error message', async () => {
        ...
        expect(result.message).toBe('Vali: Unauthorized. 401. Unauthorized');
      });
    });
    ...
    describe('and call fails with 404 error', () => {
      it('should return error status and a detailed error message', async () => {
        ...
        expect(result.status).toEqual('error');
        expect(result.message).toBe('Vali: Not found. 404. 404 page not found');
      });
    });
    ...
    describe('and call fails with 502 error', () => {
      it('should return error status and a detailed error message', async () => {
        ...
        expect(result.status).toEqual('error');
        expect(result.message).toBe('Vali: Bad Gateway. 502');
      });
    });
  });
  ...
  describe('when calling annotationQuery', () => {
    ...
    it('should transform the vali data to annotation response', async () => {
  ...
  describe('metricFindQuery', () => {
    const getTestContext = (mock: ValiDatasource) => {
      const ds = createValiDSForTests();
    ...
    mocks.forEach((mock, index) => {
      it(`should return label names for Vali v${index}`, async () => {
    ...
    mocks.forEach((mock, index) => {
      it(`should return label values for Vali v${index}`, async () => {
        const { ds } = getTestContext(mock);
    ...
    mocks.forEach((mock, index) => {
      it(`should return empty array when incorrect query for Vali v${index}`, async () => {
        const { ds } = getTestContext(mock);
...
function createValiDSForTests(
  ...
  return new ValiDatasource(customSettings, templateSrvMock, timeSrvStub as any);
}

function makeAnnotationQueryRequest(): AnnotationQueryRequest<ValiQuery> {
  const timeRange = {
    from: dateTime(),
    to: dateTime(),
  };
...
function makeMetadataAndVersionsMocks() {
  const mocks = [];
  for (let i = 0; i <= 1; i++) {
    const mock: ValiDatasource = makeMockValiDatasource({ label1: ['value1', 'value2'], label2: ['value3', 'value4'] });
