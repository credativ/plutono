...
import { toUtc } from '@credativ/plutono-data';

import { ValiQueryEditor } from './ValiQueryEditor';
...
const setup = (propOverrides?: object) => {
  ...
  const datasource: ValiDatasource = datasourceMock as ValiDatasource;
  const onRunQuery = jest.fn();
  const onChange = jest.fn();

  const query: ValiQuery = {
    expr: '',
    refId: 'A',
  ...
  const wrapper = shallow(<ValiQueryEditor {...props} />);
...
describe('Render ValiQueryEditor with legend', () => {
  it('should render', () => {
    const { wrapper } = setup();
