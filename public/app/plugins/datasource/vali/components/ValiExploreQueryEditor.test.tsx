...
import ValiExploreQueryEditor from './ValiExploreQueryEditor';
...
import ValiLanguageProvider from '../language_provider';

const setup = (renderMethod: any, propOverrides?: object) => {
  const datasource: ValiDatasource = makeMockValiDatasource({});
  datasource.languageProvider = new ValiLanguageProvider(datasource);
  const onRunQuery = jest.fn();
  const onChange = jest.fn();
  const query: ValiQuery = { expr: '', refId: 'A', maxLines: 0 };
  ...
  return renderMethod(<ValiExploreQueryEditor {...props} />);
};

describe('ValiExploreQueryEditor', () => {
  ...
  it('should render ValiQueryField with ExtraFieldElement when ExploreMode is set to Logs', async () => {
    ...
    await act(async () => {
      ...
      expect(wrapper.find(ValiOptionFields).length).toBe(1);
    });
  });
});
