...
import LanguageProvider, { ValiHistoryItem } from './language_provider';
import { TypeaheadInput } from '@credativ/plutono-ui';
...
describe('Language completion provider', () => {
  const datasource = makeMockValiDatasource({});
  ...
  describe('label key suggestions', () => {
    it('returns all label suggestions on empty selector', async () => {
      const datasource = makeMockValiDatasource({ label1: [], label2: [] });
  ...
  describe('label suggestions', () => {
    it('returns label values suggestions from Vali', async () => {
      const datasource = makeMockValiDatasource({ label1: ['label1_val1', 'label1_val2'], label2: [] });
      const provider = await getLanguageProvider(datasource);
    ...
    it('returns label values suggestions from Vali when re-editing', async () => {
      const datasource = makeMockValiDatasource({ label1: ['label1_val1', 'label1_val2'], label2: [] });
      const provider = await getLanguageProvider(datasource);
  ...
  describe('label values', () => {
    it('should fetch label values if not cached', async () => {
      const datasource = makeMockValiDatasource({ testkey: ['label1_val1', 'label1_val2'], label2: [] });
      const provider = await getLanguageProvider(datasource);
...
describe('Request URL', () => {
  it('should contain range params', async () => {
    const datasourceWithLabels = makeMockValiDatasource({ other: [] });
    ...
    const expectedUrl = '/vali/api/v1/label';
...
describe('Query imports', () => {
  ...
  describe('prometheus query imports', () => {
    ...
    it('returns selector query from selector query with common labels', async () => {
      const datasourceWithLabels = makeMockValiDatasource({ foo: [] });
      const instance = new LanguageProvider(datasourceWithLabels);
    ...
    it('returns selector query from selector query with all labels if logging label list is empty', async () => {
      const datasourceWithLabels = makeMockValiDatasource({});
      const instance = new LanguageProvider(datasourceWithLabels);
      const result = await instance.importPrometheusQuery('metric{foo="bar",baz="42"}');
...
async function getLanguageProvider(datasource: ValiDatasource) {
  const instance = new LanguageProvider(datasource);
