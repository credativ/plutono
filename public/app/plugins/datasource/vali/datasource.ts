...
import { getTemplateSrv, TemplateSrv, BackendSrvRequest, FetchError, getBackendSrv } from '@credativ/plutono-runtime';
import { addLabelToQuery } from 'app/plugins/datasource/prometheus/add_label_to_query';
import { getTimeSrv, TimeSrv } from 'app/features/dashboard/services/TimeSrv';
import { convertToWebSocketUrl } from 'app/core/utils/explore';
import {
  valiResultsToTableModel,
  valiStreamResultToDataFrame,
  valiStreamsToDataFrames,
...
import {
  ValiOptions,
  ValiQuery,
  ValiRangeQueryRequest,
  ValiResponse,
  ValiResultType,
  ValiStreamResponse,
} from './types';
import { LiveStreams, ValiLiveTarget } from './live_streams';
import LanguageProvider from './language_provider';
import { serializeParams } from '../../../core/utils/fetch';
import { RowContextOptions } from '@credativ/plutono-ui/src/components/Logs/LogRowContextProvider';
import syntax from './syntax';

export type RangeQueryOptions = DataQueryRequest<ValiQuery> | AnnotationQueryRequest<ValiQuery>;
export const DEFAULT_MAX_LINES = 1000;
export const VALI_ENDPOINT = '/vali/api/v1';
const NS_IN_MS = 1000000;

const RANGE_QUERY_ENDPOINT = `${VALI_ENDPOINT}/query_range`;
const INSTANT_QUERY_ENDPOINT = `${VALI_ENDPOINT}/query`;

const DEFAULT_QUERY_PARAMS: Partial<ValiRangeQueryRequest> = {
  direction: 'BACKWARD',
  limit: DEFAULT_MAX_LINES,
  query: '',
};

export class ValiDatasource extends DataSourceApi<ValiQuery, ValiOptions> {
  private streams = new LiveStreams();
  languageProvider: LanguageProvider;
  maxLines: number;

  constructor(
    private instanceSettings: DataSourceInstanceSettings<ValiOptions>,
  ...
  query(options: DataQueryRequest<ValiQuery>): Observable<DataQueryResponse> {
  ...
  runInstantQuery = (
    target: ValiQuery,
    options: DataQueryRequest<ValiQuery>,
    ...
    return this._request(INSTANT_QUERY_ENDPOINT, query).pipe(
      map((response: { data: ValiResponse }) => {
        if (response.data.data.resultType === ValiResultType.Stream) {
          return {
            data: response.data
              ? valiStreamsToDataFrames(
                  response.data as ValiStreamResponse,
                  target,
                  query.limit,
        ...
        return {
          data: [valiResultsToTableModel(response.data.data.result, responseListLength, target.refId, meta, true)],
          key: `${target.refId}_instant`,
        };
      }),
  ...
  createRangeQuery(target: ValiQuery, options: RangeQueryOptions, limit: number): ValiRangeQueryRequest {
    ...
    if (options.range) {
      ...
      const adjustedInterval =
        this.adjustInterval((options as DataQueryRequest<ValiQuery>).intervalMs || 1000, rangeMs) / 1000;
  ...
  /**
   * Attempts to send a query to /vali/api/v1/query_range
   */
  runRangeQuery = (
    target: ValiQuery,
    ...
    let maxDataPoints = isMetricsQuery(target.expr)
      ? // We fallback to maxLines here because maxDataPoints is defined as possibly undefined. Not sure that can
        ...
        (options as DataQueryRequest<ValiQuery>).maxDataPoints || this.maxLines
    ...
    if ((options as DataQueryRequest<ValiQuery>).liveStreaming) {
    ...
    return this._request(RANGE_QUERY_ENDPOINT, query).pipe(
      catchError((err: any) => this.throwUnless(err, err.status === 404, target)),
      switchMap((response: { data: ValiResponse; status: number }) =>
        processRangeQueryResponse(
          response.data,
          target,
          query,
          ...
          maxDataPoints,
          this.instanceSettings.jsonData,
          (options as DataQueryRequest<ValiQuery>).scopedVars,
          (options as DataQueryRequest<ValiQuery>).reverse
        )
      )
    );
  };

  createLiveTarget(target: ValiQuery, maxDataPoints: number): ValiLiveTarget {
    const query = target.expr;
    const baseUrl = this.instanceSettings.url;
    ...
    return {
      query,
      url: convertToWebSocketUrl(`${baseUrl}/vali/api/v1/tail?${params}`),
      refId: target.refId,
      size: maxDataPoints,
    };
  }
   ...
   * Vali streams, sets only common labels on dataframe.labels and has additional dataframe.fields.labels for unique
   * labels per row.
   */
  runLiveQuery = (target: ValiQuery, maxDataPoints: number): Observable<DataQueryResponse> => {
    const liveTarget = this.createLiveTarget(target, maxDataPoints);

    return this.streams.getStream(liveTarget).pipe(
      map((data) => ({
        data: data || [],
        key: `vali-${liveTarget.refId}`,
  ...
  interpolateVariablesInQueries(queries: ValiQuery[], scopedVars: ScopedVars): ValiQuery[] {
  ...
  getQueryDisplayText(query: ValiQuery) {
    return query.expr;
  }
  ...
  async importQueries(queries: ValiQuery[], originMeta: PluginMeta): Promise<ValiQuery[]> {
  ...
  interpolateQueryExpr(value: any, variable: any) {
    ...
    if (!variable.multi && !variable.includeAll) {
      return valiRegularEscape(value);
    }

    if (typeof value === 'string') {
      return valiSpecialRegexEscape(value);
    }

    const escapedValues = lodashMap(value, valiSpecialRegexEscape);
    return escapedValues.join('|');
  }

  modifyQuery(query: ValiQuery, action: any): ValiQuery {
  ...
  getHighlighterExpression(query: ValiQuery): string[] {
  ...
  getLogRowContext = (row: LogRowModel, options?: RowContextOptions): Promise<{ data: DataFrame[] }> => {
    ...
    return this._request(RANGE_QUERY_ENDPOINT, target)
      .pipe(
        ...
        switchMap((res: { data: ValiStreamResponse; status: number }) =>
          of({
            data: res.data ? res.data.data.result.map((stream) => valiStreamResultToDataFrame(stream, reverse)) : [],
          })
        )
      )
      .toPromise();
  ...
  prepareLogRowContextQueryTarget = (row: LogRowModel, limit: number, direction: 'BACKWARD' | 'FORWARD') => {
    ...
    if (direction === 'BACKWARD') {
      ...
      return {
        ...commonTargetOptions,
        // start param in Vali API is inclusive so we'll have to filter out the row that this request is based from
  ...
  testDatasource() {
    ...
    return this._request(`${VALI_ENDPOINT}/label`, { start })
      .pipe(
        map((res) => {
          ...
          const testResult =
            values.length > 0
              ...
              : {
                  status: 'error',
                  message:
                    'Data source connected, but no labels received. Verify that Vali and Promtail is configured properly.',
                };
          return testResult;
        }),
        catchError((err: any) => {
          ...
          if (err.statusText) {
            ...
            message += 'Cannot connect to Vali';
          }
  ...
  async annotationQuery(options: AnnotationQueryRequest<ValiQuery>): Promise<AnnotationEvent[]> {
  ...
  throwUnless(err: FetchError, condition: boolean, target: ValiQuery) {
    if (condition) {
      return of(err);
    }
  ...
  processError(err: FetchError, target: ValiQuery) {
  ...
  adjustInterval(interval: number, range: number) {
    // Vali will drop queries that might return more than 11000 data points.
...
export function valiRegularEscape(value: any) {
  if (typeof value === 'string') {
...
export function valiSpecialRegexEscape(value: any) {
  if (typeof value === 'string') {
    return valiRegularEscape(value.replace(/\\/g, '\\\\\\\\').replace(/[$^*{}\[\]+?.()|]/g, '\\\\$&'));
  }
  return value;
}
