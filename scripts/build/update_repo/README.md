...
```bash
docker run -ti --rm -u 0:0 plutono/plutono-ci-deploy:1.2.3 bash # 1.2.3 is the newest image at the time of writing
# in the container:
mkdir -p /dist

#outside of container:
cd <plutono project dir>/..
docker cp plutono <container_name>:/
docker cp <gpg.key used for signing> <container_name>:/private.key

#in container:
./scripts/build/update_repo/load-signing-key.sh
cd dist && wget https://dl.grafana.com/oss/release/plutono_5.4.3_amd64.deb && wget https://dl.grafana.com/oss/release/plutono-5.4.3-1.x86_64.rpm && cd ..
