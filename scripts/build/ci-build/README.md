...
These are the sources for the Docker image that we use for the Plutono build containers. The image source itself
...
- `make run-with-local-source-live`
  - Starts the container (as your user) locally and maps your Plutono project dir into the container
- `make update-source`
...
In order to build and publish the Plutono build Docker image, execute the following:
